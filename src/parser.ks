...
class Parser {
	...
	reqClassAbstractMethod(attributes?, modifiers, first?) ~ SyntaxError { // {{{
		...
		return this.yep(AST.MethodDeclaration(attributes, modifiers, name, parameters, type, throws, null, first, throws ?? type ?? mods ?? parameters))
	} // }}}
	...
	reqClassMember(attributes?, modifiers, first?) ~ SyntaxError { // {{{
		...
		if this.match(Token::COLON, Token::LEFT_CURLY, Token::LEFT_ROUND) == Token::COLON {
			...
			if this.test(Token::LEFT_CURLY) {
				...
				return this.reqClassField(attributes, modifiers, name, type, first ?? name)
			}
		}
			...
			return this.reqClassField(attributes, modifiers, name, null, first ?? name)
		}
	} // }}}
	...
	reqExportDeclarator() ~ SyntaxError { // {{{
		if this.matchM(M.EXPORT_STATEMENT) == Token::CLASS {
	...
	reqExternNamespaceDeclaration(first, modifiers = []) ~ SyntaxError { // {{{
		...
		if this.test(Token::LEFT_CURLY) {
			...
			until this.test(Token::RIGHT_CURLY) {
				statements.push(this.reqExternDeclarator())
	...
	reqFunctionModifiers() ~ SyntaxError { // {{{
	...
	reqFunctionStatement(first) ~ SyntaxError { // {{{
		const identifier = this.reqIdentifier()
		const parameters = this.reqFunctionParameterList()
		const modifiers = this.reqFunctionModifiers()
		const type = this.reqFunctionReturns()
		...
		return this.yep(AST.FunctionDeclaration(identifier, parameters, modifiers, type, throws, body, first, body))
	} // }}}
	...
	reqImportDeclarator() ~ SyntaxError { // {{{
		const specifiers = [this.reqImportSpecifier()]
		
		while this.test(Token::COMMA) {
			this.commit()
			
			specifiers.push(this.reqImportSpecifier())
		}
		
		unless this.test(Token::FROM) {
			this.throw('from')
		}
		...
		if this.test(Token::WITH) {
			...
			references = [this.reqImportReference()]
			
			while this.test(Token::COMMA) {
				this.commit()
				
				references.push(this.reqImportReference())
			}
			
			last = references[references.length - 1]
		}
		
		return this.yep(AST.ImportDeclarator(module, specifiers, references, specifiers[0], last))
	} // }}}
	reqImportReference() ~ SyntaxError { // {{{
		const alias = this.reqIdentifier()
		
		if this.test(Token::AS) {
			this.commit()
			
			return this.yep(AST.ImportReference(alias, this.reqIdentifier()))
		}
		else {
			return this.yep(AST.ImportReference(alias))
		}
	} // }}}
	reqImportSpecifier() ~ SyntaxError { // {{{
		if this.test(Token::ASTERISK) {
			...
			if this.test(Token::AS) {
				...
				const local = this.reqIdentifier()
				
				return this.yep(AST.ImportWildcardSpecifier(local, first, local))
			}
			else {
				return this.yep(AST.ImportWildcardSpecifier(null, first, first))
			}
		}
		else {
			const alias = this.reqIdentifier()
			...
			if this.test(Token::AS) {
				...
				return this.yep(AST.ImportSpecifier(alias, local, alias, local))
			}
			else {
				return this.yep(AST.ImportSpecifier(alias, null, alias, alias))
			}
		}
	} // }}}
	...
	reqObjectItem() ~ SyntaxError { // {{{
		...
		if this.test(Token::COLON) {
			...
			return this.yep(AST.ObjectMember(attributes, name, this.yep(AST.FunctionExpression(parameters, modifiers, type, body, parameters, body)), first ?? name, body))
		}
	} // }}}
	...
	reqTemplateExpression() ~ SyntaxError { // {{{
		const first = this.yes()
	...
	reqTypeVar() ~ SyntaxError { // {{{
		if this.match(Token::LEFT_CURLY, Token::LEFT_SQUARE) == Token::LEFT_CURLY {
			...
			let identifier, parameters, modifiers, type
			until this.test(Token::RIGHT_CURLY) {
				if this.test(Token::IDENTIFIER) {
					identifier = this.reqIdentifier()
					...
					if this.test(Token::COLON) {
						...
						type = this.yep(AST.FunctionExpression(parameters, modifiers, type, null, parameters, type ?? modifiers ?? parameters))
					}
