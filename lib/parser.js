...
module.exports = function() {
	...
	class Parser {
		...
		__ks_func_reqExportDeclarator_0() {
			let __ks_0 = this.matchM(M.EXPORT_STATEMENT);
		...
		__ks_func_reqExternNamespaceDeclarator_0() {
			let __ks_0 = this.matchM(M.EXTERN_STATEMENT);
			if(__ks_0 === Token.ABSTRACT) {
				const __ks_abstract_1 = this.yep(AST.Modifier(ModifierKind.Abstract, this.yes()));
				if(this.test(Token.CLASS)) {
		...
		reqExternNamespaceDeclarator() {
			if(arguments.length === 0) {
				return Parser.prototype.__ks_func_reqExternNamespaceDeclarator_0.apply(this);
			}
			throw new SyntaxError("wrong number of arguments");
		}
				...
				while(!(this.test(Token.RIGHT_ROUND))) {
					if(this.test(Token.IDENTIFIER)) {
						...
						if(this.test(Token.COLON)) {
							...
							__ks_arguments_1.push(this.yep(AST.NamedArgument(name, value)));
						}
						else {
							__ks_arguments_1.push(this.reqOperation(ExpressionMode.Default, name));
						}
					}
					else {
						__ks_arguments_1.push(this.reqOperation(ExpressionMode.Default));
					}
					if(this.test(Token.COMMA)) {
						this.commit();
					}
