...
module.exports = function() {
	...
	var {AssignmentOperatorKind, BinaryOperatorKind, ModifierKind, NodeKind, ScopeKind, UnaryOperatorKind} = require("@kaoscript/ast")();
	let AST = (function() {
		const $polyadic = {};
	...
	let Token = {
		...
		EXCLAMATION_QUESTION_EQUALS: 46,
		EXPORT: 47,
		EXTENDS: 48,
		EXTERN: 49,
		EXTERN_REQUIRE: 50,
		...
		FUNC: 54,
		GET: 55,
		HASH_EXCLAMATION_LEFT_SQUARE: 56,
		HASH_LEFT_SQUARE: 57,
		HEX_NUMBER: 58,
		IDENTIFIER: 59,
		IF: 60,
		...
		IS_NOT: 68,
		LEFT_ANGLE: 69,
		LEFT_ANGLE_EQUALS: 70,
		LEFT_ANGLE_LEFT_ANGLE: 71,
		LEFT_ANGLE_LEFT_ANGLE_EQUALS: 72,
		LEFT_CURLY: 73,
		LEFT_ROUND: 74,
		LEFT_SQUARE: 75,
		LET: 76,
		...
		QUESTION_EQUALS: 98,
		QUESTION_DOT: 99,
		QUESTION_LEFT_ROUND: 100,
		QUESTION_LEFT_SQUARE: 101,
		QUESTION_QUESTION: 102,
		...
		RETURN: 109,
		RIGHT_ANGLE: 110,
		RIGHT_ANGLE_EQUALS: 111,
		RIGHT_ANGLE_RIGHT_ANGLE: 112,
		RIGHT_ANGLE_RIGHT_ANGLE_EQUALS: 113,
		RIGHT_CURLY: 114,
		RIGHT_ROUND: 115,
		RIGHT_SQUARE: 116,
		SEALED: 117,
	...
	let M = (function() {
		...
		function MODULE_STATEMENT(that, index) {
			...
			if(c === -1) {
				...
				if((that.charAt(1) === 109) && (that.charAt(2) === 112) && (that.charAt(3) === 111) && (that.charAt(4) === 114) && (that.charAt(5) === 116) && that.isBoundary(6)) {
					that.next(6);
					return Token.IMPORT;
				}
				else if((that.charAt(1) === 110) && (that.charAt(2) === 99) && (that.charAt(3) === 108) && (that.charAt(4) === 117) && (that.charAt(5) === 100) && (that.charAt(6) === 101)) {
	...
	class Parser {
		...
		__ks_func_reqExpression_0(mode) {
			if(arguments.length < 1) {
				throw new SyntaxError("wrong number of arguments (" + arguments.length + " for 1)");
			}
			if(mode === void 0 || mode === null) {
			...
			if(this.match(Token.COLON, Token.COMMA, Token.IDENTIFIER, Token.RIGHT_ROUND) === Token.COLON) {
				...
				if(this.match(Token.COLON, Token.EQUALS, Token.QUESTION) === Token.COLON) {
					...
					if(this.test(Token.EQUALS)) {
						this.commit();
						const defaultValue = this.reqExpression(ExpressionMode.Default);
						parameters.push(this.yep(AST.Parameter(identifier, type, modifiers, defaultValue, Type.isValue(first) ? first : identifier, defaultValue)));
					}
					else {
						parameters.push(this.yep(AST.Parameter(identifier, type, modifiers, null, Type.isValue(first) ? first : identifier, type)));
					}
				}
				else if(this._token === Token.EQUALS) {
					this.commit();
					const defaultValue = this.reqExpression(ExpressionMode.Default);
					parameters.push(this.yep(AST.Parameter(identifier, null, modifiers, defaultValue, Type.isValue(first) ? first : identifier, defaultValue)));
				}
					...
					parameters.push(this.yep(AST.Parameter(identifier, null, modifiers, null, Type.isValue(first) ? first : identifier, identifier)));
				}
				if(this.test(Token.COMMA)) {
					this.commit();
				}
